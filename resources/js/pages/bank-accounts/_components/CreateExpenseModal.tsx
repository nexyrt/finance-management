...
export default function CreateExpenseModal({ open, onOpenChange, defaultAccountId, accounts, onCreated }: Props) {
    ...
    useEffect(() => {
        if (!open) return;
        fetch('/api/transaction-categories?type=debit', { headers: { Accept: 'application/json' } })
            .then(r => r.json())
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        form.append('transaction_type', 'debit');
        ...
        try {
            ...
            toast.success(data.message ?? 'Pengeluaran berhasil disimpan.');
            reset();
            onCreated(accountId);
            onOpenChange(false);
            ...
            <DialogContent size="2xl">
                <DialogHeader>
                    <div className="flex items-center gap-4 my-3">
                        <div className="h-12 w-12 bg-rose-50 dark:bg-rose-900/20 rounded-xl flex items-center justify-center">
                            <ArrowDownRight className="w-6 h-6 text-rose-600 dark:text-rose-400" />
                        </div>
                        <div>
                            <DialogTitle className="text-xl font-bold text-dark-900 dark:text-dark-50">Tambah Pengeluaran</DialogTitle>
                            <p className="text-sm text-dark-500 dark:text-dark-400">Catat transaksi debit pada rekening</p>
                        </div>
                    </div>
                </DialogHeader>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className={[
                                    ...
                                    />
                                    <label htmlFor="expense-attachment" className="cursor-pointer">
                                        {attachment ? (
