...
<div x-data="tallstackui_formCurrency(
    ...
    @js($property),
    @js($value),
    'id-ID' // force locale to id-ID
)">

    <x-dynamic-component :component="TallStackUi::prefix('input')" {{ $attributes->whereDoesntStartWith('wire:model') }}
        class="appearance-number-none" inputmode="numeric" :$label :$hint :$invalidate :alternative="$property" x-on:input="sync"
        x-model="input">
                ...
                @if ($clearable)
                    <div @class([
                        ...
                        <button type="button" class="cursor-pointer" dusk="tallstackui_form_currency_clearable">
                            <x-dynamic-component :component="TallStackUi::prefix('icon')" :icon="TallStackUi::icon('x-mark')" x-on:click="clear()" internal
                                @class([
