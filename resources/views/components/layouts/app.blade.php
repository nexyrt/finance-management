...
<body class="min-h-screen bg-gray-50 dark:bg-[#111113] text-gray-900 dark:text-gray-100 antialiased"
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside
            ...
            {{-- Brand --}}
            <div class="h-14 flex items-center gap-2.5 px-3 shrink-0 border-b border-gray-100 dark:border-white/6">
                <div class="w-8 h-8 rounded-lg bg-primary-600 dark:bg-primary-500 flex items-center justify-center shrink-0 shadow-sm shadow-primary-600/30">
            ...
            {{-- User profile --}}
            <div class="shrink-0 border-t border-gray-100 dark:border-white/6 p-2"
                x-data="{ userMenu: false }">
                <div class="relative">
                    <button @click="userMenu = !userMenu"
                        class="flex items-center gap-2.5 w-full p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-white/4 transition-colors text-left"
                        :title="sidebarCollapsed ? '{{ auth()->user()->name ?? 'User' }}' : undefined">
                        {{-- Avatar --}}
                        <div class="w-7 h-7 rounded-full bg-primary-100 dark:bg-primary-900/40 flex items-center justify-center shrink-0 text-primary-700 dark:text-primary-300 text-xs font-bold">
                        ...
                        <svg class="user-info w-3.5 h-3.5 text-gray-400 dark:text-zinc-500 shrink-0 transition-transform duration-150" :class="{ 'rotate-180': userMenu }" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                    ...
                    <div x-show="userMenu" x-cloak @click.away="userMenu = false"
                        ...
                        {{-- User info header --}}
                        <div class="px-3 py-2.5 border-b border-gray-100 dark:border-white/6">
                        ...
                        <div class="p-1">
                            <a href="{{ route('settings.profile') }}" wire:navigate
                                class="flex items-center gap-2 px-2.5 py-1.5 text-xs text-gray-700 dark:text-zinc-300 hover:bg-gray-50 dark:hover:bg-white/6 rounded-lg transition-colors">
                        ...
                        <div class="p-1 border-t border-gray-100 dark:border-white/6">
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            <header class="h-14 shrink-0 flex items-center gap-3 px-4 md:px-6
                ...
                <button @click="sidebarOpen = true"
                    class="lg:hidden p-1.5 rounded-lg text-gray-500 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-white/6 transition-colors shrink-0">
                ...
                <nav class="flex items-center gap-1 text-sm flex-1 overflow-x-auto scrollbar-none">
                    ...
                    @foreach ($breadcrumbs as $i => $crumb)
                        <svg class="breadcrumb-sep w-3 h-3 text-gray-300 dark:text-zinc-700 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 18l6-6-6-6"/>
                        </svg>

                        @if ($loop->last)
                            <span class="text-gray-700 dark:text-zinc-200 font-medium whitespace-nowrap text-sm shrink-0">{{ $crumb['label'] }}</span>
                        @elseif (isset($crumb['url']))
                            <a href="{{ $crumb['url'] }}" wire:navigate
                                class="text-gray-400 dark:text-zinc-500 hover:text-gray-600 dark:hover:text-zinc-300 transition-colors text-sm whitespace-nowrap shrink-0">
                                {{ $crumb['label'] }}
                            </a>
                        @else
                            <span class="text-gray-400 dark:text-zinc-600 text-sm whitespace-nowrap shrink-0">{{ $crumb['label'] }}</span>
                        @endif
                    @endforeach
                </nav>
                ...
                <div class="flex items-center gap-1 shrink-0">
                    ...
                    <button @click="darkTheme = !darkTheme"
                        class="p-2 rounded-lg text-gray-500 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-white/6 transition-colors"
