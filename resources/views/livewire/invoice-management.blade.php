        ...
        <div
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-800 dark:text-white mt-1">{{ $this->formatCurrency($this->totalRevenue) }}</p>
        ...
        <div
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-800 dark:text-white mt-1">{{ $this->formatCurrency($this->pendingAmount) }}</p>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div
                ...
                <div class="px-6 py-4 border-b border-gray-100 dark:border-zinc-800">
                    ...
                    <div class="flex items-center space-x-4">
                        <flux:select wire:model.live="filterStatus" placeholder="Semua Status" size="sm" clearable>
                        ...
                        <flux:select wire:model.live="filterClient" placeholder="Semua Klien" size="sm" clearable>
                    ...
                    <div class="space-y-4">
                                ...
                                <div class="ml-8">
                                    <div class="flex items-start justify-between">
                                        <div class="flex-1">
                                            <div class="flex items-center space-x-3 mb-2">
                                                ...
                                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $this->getStatusClass($invoice->status) }}">
                                            ...
                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                                <div>
                                                    <p class="text-gray-500 dark:text-zinc-400">Klien:</p>
                                                    <p class="font-medium text-gray-800 dark:text-white">{{ $invoice->client->name }}</p>
                                                </div>
                                                <div>
                                                    <p class="text-gray-500 dark:text-zinc-400">Total:</p>
                                                    <p class="font-bold text-gray-800 dark:text-white text-lg">{{ $this->formatCurrency($invoice->total_amount) }}</p>
                                                </div>
                                                <div>
                                                    <p class="text-gray-500 dark:text-zinc-400">Tanggal:</p>
                                                    <p class="text-gray-800 dark:text-white">{{ Carbon\Carbon::parse($invoice->issue_date)->format('d/m/Y') }}</p>
                                                </div>
                                                <div>
                                                    <p class="text-gray-500 dark:text-zinc-400">Jatuh Tempo:</p>
                                                    <p class="text-gray-800 dark:text-white">{{ Carbon\Carbon::parse($invoice->due_date)->format('d/m/Y') }}</p>
                                                </div>
                                            </div>
                                            ...
                                            @if ($invoice->status === 'partially_paid' || $invoice->status === 'paid')
                                                <div class="mt-3 p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                                                    <div class="flex justify-between text-sm">
                                                        ...
                                                        <span class="font-medium text-green-700 dark:text-green-300">{{ $this->formatCurrency($invoice->amount_paid) }}</span>
                                                    </div>
                                                    @if ($invoice->status === 'partially_paid')
                                                        <div class="flex justify-between text-sm">
                                                            <span class="text-amber-600 dark:text-amber-400">Sisa:</span>
                                                            <span class="font-medium text-amber-700 dark:text-amber-300">{{ $this->formatCurrency($invoice->amount_remaining) }}</span>
                                                        </div>
                                                    @endif
                                                </div>
                                            @endif
                                        ...
                                        <div class="relative">
                                            <flux:dropdown position="bottom" align="end">
                                                ...
                                                <flux:menu>
                                                    <flux:menu.item wire:click="openDetailModal({{ $invoice->id }})" icon="eye">
                                                        Lihat Detail
                                                    </flux:menu.item>
                                                    <flux:menu.item wire:click="openPreviewModal({{ $invoice->id }})" icon="document-text">
                                                        Preview
                                                    </flux:menu.item>
                                                    @if ($invoice->status !== 'paid')
                                                        <flux:menu.item wire:click="openPaymentModal({{ $invoice->id }})" icon="credit-card">
                                                            Catat Pembayaran
                                                        </flux:menu.item>
                                                    @endif
                                                    <flux:menu.separator />
                                                    <flux:menu.item wire:click="openEditInvoiceModal({{ $invoice->id }})" icon="pencil">
                                                        Edit Invoice
                                                    </flux:menu.item>
                                                    ...
                                                    @endif
                                                    <flux:menu.separator />
                                                    <flux:menu.item wire:click="confirmDeleteInvoice({{ $invoice->id }})" icon="trash" variant="danger">
                                                        Hapus Invoice
                                                    </flux:menu.item>
                                                </flux:menu>
                                            </flux:dropdown>
                                    ...
                                    <div class="flex items-center justify-between mt-4 pt-4 border-t border-gray-200 dark:border-zinc-700">
                                        <div class="flex space-x-2">
                                            ...
                                            @endif
                                            <button wire:click="openPreviewModal({{ $invoice->id }})"
                                        ...
                                        <div class="text-right text-xs text-gray-500 dark:text-zinc-400">
                                            <p>{{ Carbon\Carbon::parse($invoice->created_at)->diffForHumans() }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        @empty
                            <div class="text-center py-12">
                                ...
                                <p class="text-gray-500 dark:text-zinc-400 mb-4">Mulai dengan membuat invoice pertama Anda</p>
                                <button wire:click="openAddInvoiceModal"
        ...
        <div class="space-y-6">
            ...
            <div
                ...
                <div class="p-6 space-y-4">
                    @forelse($this->recentInvoices as $invoice)
                        <div
                            class="flex items-center space-x-3 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-zinc-800 transition-colors cursor-pointer"
                            wire:click="openDetailModal({{ $invoice['id'] }})">
                            <div
                                class="p-2 rounded-lg 
                                {{ $invoice['status'] === 'paid' 
                                    ? 'bg-green-100 dark:bg-green-900/30' 
                                    : ($invoice['is_overdue'] ? 'bg-red-100 dark:bg-red-900/30' : 'bg-blue-100 dark:bg-blue-900/30') }}">
                                @if ($invoice['status'] === 'paid')
                                    <svg xmlns="http://www.w3.org/2000/svg"
                            ...
                            <div class="space-y-2">
                                <div class="flex justify-between items-center">
                                    <div class="flex items-center space-x-2">
                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $this->getStatusClass($status) }}">
                                            {{ $this->getStatusLabel($status) }}
                                        </span>
                                        <span class="text-sm text-gray-600 dark:text-zinc-300">{{ $stat['count'] }}</span>
                                    </div>
                                ...
                                <div class="w-full bg-gray-200 dark:bg-zinc-700 rounded-full h-2">
                                    <div class="h-2 rounded-full
                                        {{ $status === 'paid' ? 'bg-green-500 dark:bg-green-400' : 
                                           ($status === 'overdue' ? 'bg-red-500 dark:bg-red-400' : 
                                           ($status === 'partially_paid' ? 'bg-yellow-500 dark:bg-yellow-400' : 
                                           ($status === 'sent' ? 'bg-blue-500 dark:bg-blue-400' : 'bg-gray-500 dark:bg-gray-400'))) }}"
                    ...
                    <div class="pt-4 border-t border-gray-100 dark:border-zinc-800">
                        <div class="space-y-2">
                            <div class="flex justify-between items-center">
                                ...
                                <span class="text-sm font-semibold text-gray-800 dark:text-white">{{ $this->totalInvoices }}</span>
                            </div>
                            <div class="flex justify-between items-center">
            ...
            <div
                ...
                <div class="p-6 space-y-4">
                    @php
                        $paymentMethods = \App\Models\Payment::selectRaw('payment_method, COUNT(*) as count, SUM(amount) as total')
        ...
        <form wire:submit="saveInvoice" class="space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <flux:input wire:model="invoice_number" label="Nomor Invoice" placeholder="INV-2024-0001" required />
                    
                    <flux:select wire:model.live="billed_to_id" label="Klien" placeholder="Pilih Klien" required>
                        @foreach ($this->availableClients as $client)
                            <flux:select.option value="{{ $client->id }}">
                                {{ $client->name }} ({{ $client->type === 'individual' ? 'Individual' : 'Perusahaan' }})
                            </flux:select.option>
                        @endforeach
                    </flux:select>
                ...
                <div class="space-y-4">
                    ...
                    <div class="bg-gray-50 dark:bg-zinc-800 rounded-lg p-4">
                        ...
                        <flux:input wire:model="total_amount" label="Total Amount" type="number" step="0.01" min="0" 
                            placeholder="0.00" :disabled="$autoCalculateTotal" />
                    ...
                    @foreach($invoiceItems as $index => $item)
                        <div class="bg-gray-50 dark:bg-zinc-800 rounded-lg p-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <flux:select wire:model.live="invoiceItems.{{ $index }}.client_id" label="Klien" required>
                                    @foreach ($this->availableClients as $client)
                                        <flux:select.option value="{{ $client->id }}">{{ $client->name }}</flux:select.option>
                                    @endforeach
                                </flux:select>
                                ...
                                <flux:input wire:model.live="invoiceItems.{{ $index }}.amount" 
                                    label="Jumlah" type="number" step="0.01" min="0.01" placeholder="0.00" required />
                            </div>
                        </div>
                    @endforeach
                </div>
        ...
        <form wire:submit="savePayment" class="space-y-6">
                ...
                <div class="bg-gray-50 dark:bg-zinc-800 rounded-lg p-4">
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            ...
                            <p class="font-bold text-gray-800 dark:text-white text-lg">{{ $this->formatCurrency($paymentInvoice->total_amount) }}</p>
                        </div>
                        <div>
                            <p class="text-gray-500 dark:text-zinc-400">Sudah Dibayar:</p>
                            <p class="font-medium text-green-600 dark:text-green-400">{{ $this->formatCurrency($paymentInvoice->amount_paid) }}</p>
                        </div>
                        <div>
                            <p class="text-gray-500 dark:text-zinc-400">Sisa Tagihan:</p>
                            <p class="font-bold text-red-600 dark:text-red-400 text-lg">{{ $this->formatCurrency($paymentInvoice->amount_remaining) }}</p>
                        </div>
                        <div>
                            <p class="text-gray-500 dark:text-zinc-400">Status:</p>
                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $this->getStatusClass($paymentInvoice->status) }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <flux:input wire:model="payment_amount" label="Jumlah Pembayaran" type="number" step="0.01" min="0.01" 
                    ...
                    @if ($payment_method === 'bank_transfer')
                        <flux:select wire:model="bank_account_id" label="Akun Bank" placeholder="Pilih Akun Bank" required>
                    ...
                    <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span class="font-bold text-blue-700 dark:text-blue-300">{{ $this->formatCurrency($payment_amount) }}</span>
                            </div>
                            @if ($paymentInvoice)
                                <div class="flex justify-between">
                                ...
                                @if ($payment_amount >= $paymentInvoice->amount_remaining)
                                    <div class="bg-green-100 dark:bg-green-900/30 p-2 rounded text-center">
                                        <span class="text-green-700 dark:text-green-400 font-medium">✓ Invoice akan lunas</span>
                                    </div>
                                @elseif ($payment_amount > 0)
                                    <div class="bg-yellow-100 dark:bg-yellow-900/30 p-2 rounded text-center">
                                        <span class="text-yellow-700 dark:text-yellow-400 font-medium">⚠ Pembayaran sebagian</span>
                                    </div>
                                @endif
                            @endif
                        </div>
        ...
        @if ($invoiceDetail)
            <div class="space-y-6">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-3">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $this->getStatusClass($invoiceDetail->status) }}">
                    ...
                    <div class="bg-gray-50 dark:bg-zinc-800 rounded-lg p-4">
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span class="text-gray-800 dark:text-white font-mono">{{ $invoiceDetail->invoice_number }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-500 dark:text-zinc-400">Tanggal:</span>
                                <span class="text-gray-800 dark:text-white">{{ Carbon\Carbon::parse($invoiceDetail->issue_date)->format('d/m/Y') }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-500 dark:text-zinc-400">Jatuh Tempo:</span>
                                <span class="text-gray-800 dark:text-white">{{ Carbon\Carbon::parse($invoiceDetail->due_date)->format('d/m/Y') }}</span>
                            </div>
                            <div class="flex justify-between">
                    ...
                    <div class="bg-gray-50 dark:bg-zinc-800 rounded-lg p-4">
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span class="text-gray-800 dark:text-white font-bold">{{ $this->formatCurrency($invoiceDetail->total_amount) }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-500 dark:text-zinc-400">Dibayar:</span>
                                <span class="text-green-600 dark:text-green-400 font-medium">{{ $this->formatCurrency($invoiceDetail->amount_paid) }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-500 dark:text-zinc-400">Sisa:</span>
                                <span class="text-red-600 dark:text-red-400 font-medium">{{ $this->formatCurrency($invoiceDetail->amount_remaining) }}</span>
                            </div>
                            <div class="flex justify-between pt-2 border-t border-gray-200 dark:border-zinc-700">
                                ...
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $this->getStatusClass($invoiceDetail->status) }}">
                                    {{ $this->getStatusLabel($invoiceDetail->status) }}
                                </span>
                            </div>
                        </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm">
                            <thead class="bg-gray-100 dark:bg-zinc-700">
                                <tr>
                                    <th class="px-4 py-2 text-left font-medium text-gray-700 dark:text-zinc-300">Klien</th>
                                    <th class="px-4 py-2 text-left font-medium text-gray-700 dark:text-zinc-300">Layanan</th>
                                    <th class="px-4 py-2 text-right font-medium text-gray-700 dark:text-zinc-300">Jumlah</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200 dark:divide-zinc-600">
                                @foreach($invoiceDetail->items as $item)
                                    <tr>
                                        <td class="px-4 py-2 text-gray-800 dark:text-white">{{ $item->client->name }}</td>
                                        <td class="px-4 py-2 text-gray-800 dark:text-white">{{ $item->service_name }}</td>
                                        <td class="px-4 py-2 text-right text-gray-800 dark:text-white font-medium">{{ $this->formatCurrency($item->amount) }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                            <tfoot class="bg-gray-100 dark:bg-zinc-700">
                                <tr>
                                    <td colspan="2" class="px-4 py-2 text-right font-bold text-gray-800 dark:text-white">Total:</td>
                                    <td class="px-4 py-2 text-right font-bold text-gray-800 dark:text-white">{{ $this->formatCurrency($invoiceDetail->total_amount) }}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                    ...
                    <div class="bg-gray-50 dark:bg-zinc-800 rounded-lg p-4">
                        ...
                        <div class="space-y-2">
                            @foreach($invoiceDetail->payments as $payment)
                                <div class="flex items-center justify-between p-3 bg-white dark:bg-zinc-900 rounded border">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <div>
                                            <p class="text-sm font-medium text-gray-800 dark:text-white">{{ $this->formatCurrency($payment->amount) }}</p>
                                            <p class="text-xs text-gray-500 dark:text-zinc-400">
                                    ...
                                    <div class="text-right">
                                        <p class="text-sm text-gray-800 dark:text-white">{{ Carbon\Carbon::parse($payment->payment_date)->format('d/m/Y') }}</p>
                                        @if($payment->reference_number)
                                            <p class="text-xs text-gray-500 dark:text-zinc-400">Ref: {{ $payment->reference_number }}</p>
                                        @endif
                                    </div>
                                </div>
                            @endforeach
                ...
                <div class="flex justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-zinc-700">
                    @if($invoiceDetail->status !== 'paid')
                        <flux:button type="button" variant="outline" wire:click="openPaymentModal({{ $invoiceDetail->id }})">
                            Catat Pembayaran
                        </flux:button>
                    @endif
                    <flux:button type="button" variant="outline" wire:click="openEditInvoiceModal({{ $invoiceDetail->id }})">
                        Edit Invoice
                    </flux:button>
        ...
        @if ($invoiceDetail)
            <div class="space-y-6">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-2">
                        <button class="bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 px-3 py-2 rounded-lg text-sm font-medium hover:bg-blue-200 dark:hover:bg-blue-900/50 transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
                            </svg>
                            Print
                        </button>
                        <button class="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-3 py-2 rounded-lg text-sm font-medium hover:bg-green-200 dark:hover:bg-green-900/50 transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="bg-white dark:bg-zinc-900 p-8 rounded-lg border">
                    ...
                    <div class="flex justify-between items-start mb-8">
                        <div>
                            ...
                            <p class="text-gray-600 dark:text-zinc-400 mt-2"># {{ $invoiceDetail->invoice_number }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-gray-600 dark:text-zinc-400">Tanggal: {{ Carbon\Carbon::parse($invoiceDetail->issue_date)->format('d/m/Y') }}</p>
                            <p class="text-gray-600 dark:text-zinc-400">Jatuh Tempo: {{ Carbon\Carbon::parse($invoiceDetail->due_date)->format('d/m/Y') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="bg-gray-50 dark:bg-zinc-800 p-4 rounded">
                            <p class="font-medium text-gray-800 dark:text-white">{{ $invoiceDetail->client->name }}</p>
                            ...
                            @if($invoiceDetail->client->NPWP)
                                <p class="text-gray-600 dark:text-zinc-400">NPWP: {{ $invoiceDetail->client->NPWP }}</p>
                            @endif
                        </div>
                    </div>
                        ...
                        <table class="w-full">
                            <thead>
                                <tr class="bg-gray-50 dark:bg-zinc-800">
                                    <th class="px-4 py-3 text-left font-semibold text-gray-800 dark:text-white">Deskripsi</th>
                                    <th class="px-4 py-3 text-left font-semibold text-gray-800 dark:text-white">Klien</th>
                                    <th class="px-4 py-3 text-right font-semibold text-gray-800 dark:text-white">Jumlah</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200 dark:divide-zinc-700">
                                @foreach($invoiceDetail->items as $item)
                                    <tr>
                                        <td class="px-4 py-3 text-gray-800 dark:text-white">{{ $item->service_name }}</td>
                                        <td class="px-4 py-3 text-gray-600 dark:text-zinc-400">{{ $item->client->name }}</td>
                                        <td class="px-4 py-3 text-right text-gray-800 dark:text-white">{{ $this->formatCurrency($item->amount) }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    ...
                    <div class="flex justify-end">
                        <div class="w-64">
                            <div class="bg-gray-50 dark:bg-zinc-800 p-4 rounded">
                                <div class="flex justify-between items-center text-xl font-bold text-gray-800 dark:text-white">
                                    <span>TOTAL:</span>
                                    <span>{{ $this->formatCurrency($invoiceDetail->total_amount) }}</span>
                                </div>
                                @if($invoiceDetail->amount_paid > 0)
                                    <div class="flex justify-between items-center text-sm text-green-600 dark:text-green-400 mt-2">
                                        <span>Dibayar:</span>
                    ...
                    <!-- Footer -->
                    <div class="mt-8 pt-8 border-t border-gray-200 dark:border-zinc-700 text-center text-gray-600 dark:text-zinc-400">
    ...
    <flux:modal wire:model.self="showBulkActionModal" name="bulk-action" class="md:w-96">
        <div class="space-y-6">
            <div>
                ...
                <flux:text class="mt-2">Pilih aksi untuk {{ count($selectedInvoices) }} invoice yang dipilih</flux:text>
            </div>
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            ...
                            <div class="mt-2 text-sm text-red-700 dark:text-red-300">
                                <p>Aksi ini akan menghapus semua invoice yang dipilih beserta data terkait (pembayaran, item) dan tidak dapat dikembalikan.</p>
                            </div>
                        </div>
                    </div>
                </div>
