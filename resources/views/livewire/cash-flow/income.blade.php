    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div class="space-y-1">
            ...
            <p class="text-gray-600 dark:text-zinc-400 text-lg">
                Kelola pemasukan dari pembayaran invoice dan transaksi langsung
            </p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        <x-card class="hover:shadow-lg transition-shadow">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-sm text-dark-600 dark:text-dark-400">Jumlah Transaksi</p>
    ...
    <x-table :$headers :$sort :rows="$this->rows" selectable selectable-property="uid" wire:model="selected" paginate loading>
        ...
        @interact('column_source_type', $row)
            @if ($row->source_type === 'payment')
                <x-badge text="Invoice" color="blue" icon="document-text" size="sm" />
            @else
                <x-badge text="Langsung" color="green" icon="banknotes" size="sm" />
            @endif
        @endinteract
        ...
        @interact('column_category_label', $row)
            @if ($row->category_label)
                <div class="inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg">
                    ...
                    <span class="text-xs font-medium text-purple-700 dark:text-purple-300">{{ $row->category_label }}</span>
                </div>
            @else
