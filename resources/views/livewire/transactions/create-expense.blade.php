<div>
    {{-- Trigger Button --}}
    <x-button wire:click="$toggle('modal')" color="red" icon="plus">
        {{ __('pages.add_expense_btn') }}
    </x-button>

    {{-- Modal --}}
    <x-modal :title="__('pages.add_expense_title')" wire="modal" size="xl" center persistent>
        <x-slot:title>
            <div class="flex items-center gap-4 my-3">
                ...
                <div>
                    <h3 class="text-xl font-bold text-dark-900 dark:text-dark-50">{{ __('pages.add_expense_title') }}</h3>
                    <p class="text-sm text-dark-600 dark:text-dark-400">{{ __('pages.add_expense_desc') }}</p>
                </div>
            </div>
        </x-slot:title>
        ...
        <form id="expense-create" wire:submit="save" class="space-y-6">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <x-input wire:model="description" :label="__('pages.description_label')" :placeholder="__('pages.expense_description_placeholder')" />
