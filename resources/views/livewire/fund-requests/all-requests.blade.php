    ...
    <div class="space-y-4">
        <div class="flex flex-col gap-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                {{-- User Filter --}}
                <x-select.styled wire:model.live="userFilter"
                                 :label="__('pages.requestor')"
                                 :options="array_merge(
                                     [['label' => __('pages.all_requestors'), 'value' => '']],
                                     $this->users->map(fn($user) => ['label' => $user->name, 'value' => $user->id])->toArray()
                                 )" />
    ...
    <x-table :$headers :rows="$this->rows" :$sort paginate loading>
        ...
        @interact('column_user', $row)
            <div class="flex items-center gap-3">
                ...
                <div class="flex flex-col">
                    <span class="font-semibold text-dark-900 dark:text-dark-50">{{ $row->user->name }}</span>
                    <span class="text-xs text-dark-500 dark:text-dark-400">{{ $row->user->email }}</span>
                </div>
            </div>
        @endinteract
        ...
        @interact('column_actions', $row)
            <div class="flex items-center gap-1.5">
                ...
                {{-- Review (only pending) --}}
                @if ($row->canReview() && auth()->user()->can('approve fund requests'))
                ...
                @if ($row->canDisburse() && auth()->user()->can('disburse fund requests'))
