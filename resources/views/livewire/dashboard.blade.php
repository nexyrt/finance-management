<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div class="space-y-1">
            ...
            <p class="text-gray-600 dark:text-zinc-400 text-lg">Ringkasan lengkap keuangan perusahaan</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
        {{-- Total Revenue --}}
        <div
            class="bg-white dark:bg-dark-800 rounded-xl p-6 border border-dark-200 dark:border-dark-600 hover:shadow-lg transition-shadow">
            <div class="flex items-center justify-between mb-4">
                ...
                @if ($this->revenueGrowth != 0)
                    <span
                        class="text-xs font-medium px-2 py-1 rounded-full {{ $this->revenueGrowth >= 0 ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' }}">
                        {{ $this->revenueGrowth >= 0 ? '+' : '' }}{{ $this->revenueGrowth }}%
                    </span>
                @endif
            </div>
            <p class="text-sm text-dark-600 dark:text-dark-400 mb-1">Total Pendapatan</p>
            <p class="text-2xl font-bold text-dark-900 dark:text-white">{{ $this->formatCurrency($this->totalRevenue) }}
            </p>
        </div>
        ...
        <div
            ...
            <div class="flex items-center justify-between mb-4">
                ...
                @if ($this->overdueInvoices > 0)
                    <span
                        class="text-xs font-medium px-2 py-1 rounded-full bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400">
            ...
            <p class="text-2xl font-bold text-dark-900 dark:text-white">
                {{ $this->formatCurrency($this->outstandingAmount) }}</p>
        </div>
        ...
        <div
            ...
            <div class="flex items-center justify-between mb-4">
                <div
                    class="h-12 w-12 bg-emerald-100 dark:bg-emerald-900/30 rounded-xl flex items-center justify-center">
            ...
            <p class="text-2xl font-bold text-dark-900 dark:text-white">{{ $this->formatCurrency($this->grossProfit) }}
            </p>
        </div>
        ...
        <div
            ...
            <div class="flex items-center justify-between mb-4">
                ...
                @if ($this->newClientsThisMonth > 0)
                    <span
                        class="text-xs font-medium px-2 py-1 rounded-full bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                ...
                @forelse ($this->topClients as $client)
                    <div class="flex items-center justify-between p-3 bg-dark-50 dark:bg-dark-700 rounded-lg">
                        <div class="flex items-center gap-3">
                            <div
                                class="h-10 w-10 bg-gradient-to-br from-primary-400 to-primary-600 rounded-full flex items-center justify-center text-white font-bold">
                                {{ $client['rank'] }}
                            </div>
                            <div>
                                <p class="font-medium text-dark-900 dark:text-white">{{ $client['name'] }}</p>
                                <p class="text-xs text-dark-500 dark:text-dark-400 capitalize">{{ $client['type'] }}
                                </p>
                            </div>
                        </div>
                        <div class="text-right">
                            <p class="font-bold text-dark-900 dark:text-white">
                                {{ $this->formatCurrency($client['total_revenue']) }}</p>
                        </div>
                    </div>
                @empty
        ...
        <div class="bg-white dark:bg-dark-800 rounded-xl p-6 border border-dark-200 dark:border-dark-600">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('recurring-invoices.index') }}" wire:navigate
                    class="text-sm text-primary-600 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-300">
                    Kelola →
                </a>
            </div>
            <div class="space-y-4">
                <div
                    class="p-4 bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 rounded-lg">
                    <p class="text-sm text-blue-700 dark:text-blue-300 mb-1">MRR (Monthly Recurring Revenue)</p>
                    <p class="text-2xl font-bold text-blue-900 dark:text-blue-100">
                        {{ $this->formatCurrency($this->monthlyRecurringRevenue) }}</p>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="p-4 bg-dark-50 dark:bg-dark-700 rounded-lg">
                        ...
                        <p class="text-xl font-bold text-dark-900 dark:text-white">{{ $this->draftRecurringInvoices }}
                        </p>
                    </div>
                </div>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {{-- Recent Invoices --}}
        <div
            class="lg:col-span-2 bg-white dark:bg-dark-800 rounded-xl p-6 border border-dark-200 dark:border-dark-600">
            <div class="flex items-center justify-between mb-6">
            ...
            <div class="space-y-2">
                @forelse ($this->recentInvoices as $invoice)
                    <div
                        class="flex items-center justify-between p-3 hover:bg-dark-50 dark:hover:bg-dark-700 rounded-lg transition-colors">
                        <div class="flex-1">
                            <p class="font-mono font-bold text-sm text-dark-900 dark:text-white">
                                {{ $invoice['invoice_number'] }}</p>
                            <p class="text-xs text-dark-600 dark:text-dark-400">{{ $invoice['client_name'] }} •
                                {{ $invoice['issue_date'] }}</p>
                        </div>
                        <div class="flex items-center gap-3">
                            <p class="font-bold text-dark-900 dark:text-white">
                                {{ $this->formatCurrency($invoice['total_amount']) }}</p>
                ...
                <div class="bg-white dark:bg-dark-800 rounded-xl p-6 border border-dark-200 dark:border-dark-600">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            ...
                            <p class="text-xl font-bold text-dark-900 dark:text-white">
                                {{ $this->pendingReimbursements }} pengajuan</p>
                        </div>
                    </div>
                    <div class="pt-4 border-t border-dark-200 dark:border-dark-600">
                        ...
                        <p class="text-lg font-semibold text-dark-900 dark:text-white">
                            {{ $this->formatCurrency($this->pendingReimbursementAmount) }}</p>
                    </div>
                    <a href="{{ route('reimbursements.index') }}" wire:navigate
                        class="block mt-4 text-center text-sm text-primary-600 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-300 font-medium">
                        Tinjau Sekarang →
                    </a>
                </div>
            @endif
            ...
            <div class="bg-white dark:bg-dark-800 rounded-xl p-6 border border-dark-200 dark:border-dark-600">
                ...
                <div class="space-y-2">
                    <a href="{{ route('invoices.create') }}" wire:navigate
                        class="flex items-center gap-3 p-3 rounded-lg hover:bg-primary-50 dark:hover:bg-primary-900/20 transition-colors group">
                        <div
                            class="h-8 w-8 bg-primary-100 dark:bg-primary-900/30 rounded-lg flex items-center justify-center group-hover:bg-primary-200 dark:group-hover:bg-primary-900/50 transition-colors">
                    ...
                    <a href="{{ route('cash-flow.index') }}" wire:navigate
                        class="flex items-center gap-3 p-3 rounded-lg hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-colors group">
                        <div
                            class="h-8 w-8 bg-emerald-100 dark:bg-emerald-900/30 rounded-lg flex items-center justify-center group-hover:bg-emerald-200 dark:group-hover:bg-emerald-900/50 transition-colors">
                            ...
                            tooltip: {
                                ...
                                callbacks: {
                                    label: function(context) {
                                        return 'Pendapatan: Rp ' + new Intl.NumberFormat('id-ID').format(context
                                            .parsed.y);
                                    }
                                }
                            }
