...
<div class="space-y-6 animate-pulse">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div class="space-y-2">
            <div class="h-9 bg-gray-200 dark:bg-[#27272a] rounded w-48"></div>
            <div class="h-5 bg-gray-200 dark:bg-[#27272a] rounded w-72"></div>
        </div>
        <div class="flex items-center gap-2">
            <div class="h-8 bg-gray-200 dark:bg-[#27272a] rounded-lg w-28"></div>
            <div class="h-8 bg-gray-200 dark:bg-[#27272a] rounded-lg w-28"></div>
            <div class="h-8 bg-gray-200 dark:bg-[#27272a] rounded-lg w-36"></div>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            @foreach (range(1, 4) as $i)
                <div class="space-y-1.5">
                    ...
                    <div class="h-9 bg-gray-200 dark:bg-[#27272a] rounded-xl w-full"></div>
                </div>
            @endforeach
        </div>
        ...
        @foreach (range(1, 8) as $row)
            <div class="px-4 py-4 border-b border-gray-100 dark:border-white/8 flex gap-4 items-center
                ...
                @foreach (range(1, 6) as $col)
                    <div class="h-3 bg-gray-200 dark:bg-[#27272a] rounded flex-1
