<div>
    <x-modal wire="showModal" title="Edit Pembayaran" size="2xl" center id="payment-edit-modal"
             x-on:close="$wire.resetData()">
        
        @if($payment)
            {{-- Invoice Info Header --}}
            <div class="bg-gradient-to-r from-blue-50 to-primary-50 dark:from-blue-900/20 dark:to-primary-900/20 -m-4 mb-6 p-4 border-b border-blue-200 dark:border-blue-700">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="font-bold text-secondary-900 dark:text-dark-50">{{ $payment->invoice->invoice_number }}</h3>
                            <p class="text-sm text-secondary-600 dark:text-dark-400">{{ $payment->invoice->client->name }}</p>
                        </div>
                    </div>
            ...
            <div class="space-y-6">
                ...
                @if($amount)
                    ...
                    <div class="bg-secondary-50 dark:bg-dark-800 rounded-xl p-4 border border-secondary-200 dark:border-dark-700">
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium">Rp {{ number_format($payment->invoice->total_amount, 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium text-blue-600">Rp {{ number_format($amountInteger, 0, ',', '.') }}</span>
                            </div>
        ...
        <x-slot:footer>
            <div class="flex items-center justify-between w-full">
                ...
                {{-- Main Actions --}}
                <div class="flex items-center gap-3">
                    <x-button x-on:click="$modalClose('payment-edit-modal')" color="secondary">
                        Batal
                    </x-button>
                    <x-button wire:click="save" color="primary" icon="check" spinner="save">
                        Update Pembayaran
                    </x-button>
                </div>
            </div>
